...
str(dataset) #Show the attribute of the dataset


### Data cleaning ###
nrow(dataset)
dataset <- na.omit(dataset) #Omit the NA values
nrow(dataset)
...
model1 <- lm(CombinedMPG~Weight+HP+PeakRPM, data=data.train)
summary(model1) 

# Prediction model 2: General Linear Regression
model2 <- glm(CombinedMPG~Weight+HP+PeakRPM+EngineType, data=data.train)
summary(model2) 
